...
export default function TeamOperations({
  ...
  const [groupRequests, setGroupRequests] = useState(
    initialGroupRequests.filter(
      (request) =>
        ...
        request.service?.estado !== "cancelado",
    ),
  );
  ...
  const [acceptingService, setAcceptingService] = useState<Service | null>(
    null,
  );
  ...
  const [selectedEvaluationUser, setSelectedEvaluationUser] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const [detalleEmpleadaId, setDetalleEmpleadaId] = useState<string | null>(
    null,
  );
  /*
   ...
   */
  const [reprogramandoService, setReprogramandoService] =
    useState<Service | null>(null);
  const [ubicacionService, setUbicacionService] = useState<Service | null>(
    null,
  );
  ...
  const visibleEmployees = useMemo(
    () =>
      employees.filter((employee) =>
        employee.nombreArtistico.toLowerCase().includes(query.toLowerCase()),
      ),
    [employees, query],
  );
  const detalleEmpleada = detalleEmpleadaId
    ? (employees.find((employee) => employee.id === detalleEmpleadaId) ?? null)
    : null;
  /*
  ...
  const filteredHistory =
    historyEmployeeId === "all"
      ? history
      : history.filter((service) => service.empleadaId === historyEmployeeId);
  ...
  async function reloadEmployees() {
    try {
      setEmployees(await getJefeEmployees());
    } catch {
      /* silenciar error en refresco secundario */
    }
  }
  ...
  async function reloadCashSummary() {
    try {
      setCashSummary(await getJefeCashObligations());
    } catch {
      /* silenciar error en refresco secundario */
    }
  }
  ...
  async function reloadCanalSinLeer() {
    try {
      setCanalSinLeer(await getCanalSinLeer());
    } catch {
      /* silenciar error en refresco secundario */
    }
  }
        ...
        try {
          ...
          */
          if (payload.type === "trip_status_updated") {
            const { action, tripType, employeeName, driverName } =
              payload.data ?? {};
            ...
            */
            const aviso: Record<string, string> = {
              employee_arrived: vuelta
                ? `${quien} llegó a su casa`
                : `${quien} llegó al punto`,
              employee_en_route: vuelta
                ? `${quien} ya va de regreso`
                : `${quien} ya va en camino`,
              driver_arrived: `${chofer} llegó por ${quien}`,
              employee_picked_up: vuelta
                ? `${chofer} lleva a ${quien} de regreso`
                : `${chofer} recogió a ${quien}`,
              trip_finished: vuelta
                ? `${quien} ya está en su casa`
                : `${chofer} dejó a ${quien} en el destino`,
            };
          ...
          /* La siguiente actualización válida reconciliará el estado. */
        }
      };
  ...
  function toggleAvailability(employee: Employee) {
    startTransition(async () => {
      ...
      setEmployees((current) =>
        current.map((item) =>
          item.id === employee.id ? { ...item, disponible: next } : item,
        ),
      );
      toast.success(
        next
          ? "Empleada marcada como disponible"
          : "Empleada marcada como no disponible",
      );
    });
  }
  ...
  async function openChat(service: Service) {
    ...
    try {
      setMessages(await getServiceMessages(service.id));
    } catch (error) {
      toast.error(
    ...
    */}
      ...
      <div className="mb-5 grid grid-cols-3 gap-1.5">
        {(
          ...
          >
            ...
            {count !== null && count > 0 && (
              <span
                className={`inline-flex h-[18px] min-w-[18px] items-center justify-center rounded-full px-1.5 text-[10px] font-bold tabular-nums ${tab === value ? "bg-black/20 text-black" : "bg-[#C5A55A]/15 text-[#E8D5A3]"}`}
              >
                {count}
              </span>
            )}
      ...
      {tab === "historial" && (
        <label className="mb-5 block">
          <span className="mb-2 block text-[10px] font-semibold uppercase tracking-[0.2em] text-[#C5A55A]">
            Filtrar por empleada
          </span>
          <select
            value={historyEmployeeId}
            onChange={(event) => setHistoryEmployeeId(event.target.value)}
            className="w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-sm text-white outline-none focus:border-[#C5A55A] sm:max-w-sm"
          >
      ...
      {tab === "equipo" ? (
        <section>
          <label className="mb-4 flex items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-950 px-4 focus-within:border-[#C5A55A]/70">
            <Search size={18} className="text-[#C5A55A]" />
            <input
              value={query}
          ...
          <EmployeeList
            ...
            sinLeer={canalSinLeer}
            disabled={pending}
            onToggle={toggleAvailability}
            onOpen={(employee) => setDetalleEmpleadaId(employee.id)}
          />
        </section>
      ) : tab === "grupos" ? (
        <GroupServiceOrganizer initialRequests={groupRequests} />
      ) : tab === "efectivo" ? (
        <CashDeliveryPanel
          summary={cashSummary}
          pending={pending}
          run={(action) =>
            startTransition(async () => {
              ...
              setCashSummary(await getJefeCashObligations());
              toast.success("Entrega de efectivo registrada");
            })
          }
        />
      ) : tab === "historial" ? (
        <ServiceList
          employees={employees}
          services={filteredHistory}
          allServices={services}
          active={false}
          disabled={pending}
          onDecide={decide}
          onRequestAccept={setAcceptingService}
          onRequestEdit={setEditingService}
          onRequestReschedule={setReprogramandoService}
          onRequestLocation={setUbicacionService}
          onCancel={setCancellingService}
          onChat={openChat}
        ...
        <div className="flex flex-col gap-7">
          ...
          {groupRequests.length > 0 && (
            ...
            >
              <span className="min-w-0">
                <span className="block text-sm font-semibold text-[#E8D5A3]">
                  {groupRequests.length === 1
                    ? "Una solicitud de servicio grupal"
                    : `${groupRequests.length} solicitudes de servicio grupal`}
                </span>
                <span className="mt-0.5 block text-[11px] text-zinc-500">
                  Toca para organizarlas
                </span>
              </span>
          ...
          {active.length === 0 && (
            <div className="rounded-2xl border border-dashed border-zinc-800 px-6 py-14 text-center">
              <p className="font-heading text-2xl font-semibold text-zinc-300">
                Sin servicios ahora
              </p>
              <p className="mt-2 text-sm leading-relaxed text-zinc-500">
                En cuanto entre una solicitud aparece aquí arriba y te llega el
                aviso.
              </p>
            </div>
          )}
      ...
      {acceptingService && (
        <AcceptServiceDialog
          ...
          previousService={services.find(
            (item) => item.id === acceptingService.servicioPrevioId,
          )}
          disabled={pending}
          onClose={() => setAcceptingService(null)}
          onAccept={(transport, notes) =>
            decide(acceptingService, "aceptar", transport, notes)
          }
        />
      )}
      {editingService && (
        <EditPendingServiceDialog
          ...
          onSaved={handleSaveServiceEdit}
        />
      )}
      {reprogramandoService && (
        <ServiceRescheduleDialog
          serviceId={reprogramandoService.id}
          fechaActual={reprogramandoService.fechaProgramada ?? null}
          nombreEmpleada={
            reprogramandoService.empleada?.nombreArtistico || "Este servicio"
          }
          onClose={() => setReprogramandoService(null)}
          onRescheduled={reloadServices}
        />
      )}
      {ubicacionService && (
        <ServiceLocationDialog
          serviceId={ubicacionService.id}
          ubicacionActual={
            ubicacionService.locationNameSnapshot ??
            ubicacionService.locationAddressSnapshot ??
            null
          }
          latitudActual={
            ubicacionService.ubicacionClienteLat != null
              ? Number(ubicacionService.ubicacionClienteLat)
              : null
          }
          longitudActual={
            ubicacionService.ubicacionClienteLng != null
              ? Number(ubicacionService.ubicacionClienteLng)
              : null
          }
          presetLocationIdActual={ubicacionService.presetLocationId ?? null}
          onClose={() => setUbicacionService(null)}
      ...
      {cancellingService && (
        <CancelServiceDialog
          serviceLabel={
            cancellingService.empleada?.nombreArtistico || "este servicio"
          }
          disabled={pending}
          onConfirm={(reason, note) =>
            cancelService(cancellingService, reason, note)
          }
      ...
      {canalEmpleada && (
        <CanalConModelo
          employee={canalEmpleada}
          onClose={() => setCanalEmpleada(null)}
        />
      )}
      ...
      {detalleEmpleada && (
        <EmployeeSheet
          ...
          onCanal={() => {
            setDetalleEmpleadaId(null);
            setCanalEmpleada(detalleEmpleada);
          }}
         ...
         jefe en una pantalla que no pidio. */
          onPhotos={() => {
            setDetalleEmpleadaId(null);
            setPhotosEmployee(detalleEmpleada);
          }}
          onExams={() => {
            setDetalleEmpleadaId(null);
            setSelectedEvaluationUser({
              id: detalleEmpleada.usuarioId || detalleEmpleada.id,
              name: detalleEmpleada.nombreArtistico,
            });
          }}
      ...
      {photosEmployee && (
        <ExclusivePhotosPanel
          employee={photosEmployee}
          onClose={() => setPhotosEmployee(null)}
        />
      )}
    </>
  );
...
function CashDeliveryPanel({
  ...
  const groups = Object.values(
    summary.obligations
      ...
      .reduce<Record<string, typeof summary.obligations>>((result, item) => {
        (result[item.employeeId] ??= []).push(item);
        return result;
      }, {}),
  );
  if (!groups.length)
  ...
  const servicios = groups.reduce(
    (total, obligations) => total + obligations.length,
    0,
  );
  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      <header className="flex items-center justify-between gap-4 rounded-2xl border border-[#C5A55A]/40 bg-[#C5A55A]/[0.07] px-4 py-3.5">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#8B7635]">
            Pendiente por entregar
          </p>
          <p className="mt-1 font-heading text-3xl font-semibold tabular-nums text-[#E8D5A3]">
            {money(summary.total)}
          </p>
          <p className="mt-1.5 text-[11px] text-zinc-500">
            {groups.length} {groups.length === 1 ? "empleada" : "empleadas"} ·{" "}
      ...
      {groups.map((obligations) => {
        ...
        const employeeName =
          summary.employees.find((item) => item.id === employeeId)?.name ||
          "Empleada";
        const balance = obligations.reduce(
          (sum, item) => sum + Number(item.amount) - Number(item.paidAmount),
          0,
        );
        const hasProvisional = obligations.some(
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="shrink-0 font-heading text-xl font-semibold tabular-nums text-[#E8D5A3]">
                {money(balance)}
              </span>
            </div>
            ...
            <div className="mt-3 overflow-hidden rounded-xl border border-zinc-900 bg-black/50">
              {obligations.map((item) => (
                <div
                  key={item.id}
                  className="border-b border-zinc-900 px-3 py-2.5 last:border-b-0"
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 truncate text-[10px] tabular-nums text-zinc-500">
                        Cobrado {money(Number(item.customerTotal))} · Ubers -
                        {money(Number(item.uberDeduction))}
                      </p>
                    </div>
                    <div className="flex shrink-0 items-center gap-2.5">
                      <span className="text-sm font-bold tabular-nums text-white">
                        {money(Number(item.amount) - Number(item.paidAmount))}
                      </span>
                      <button
                        type="button"
                        disabled={pending || item.calculationStatus !== "ready"}
                        onClick={() =>
                          run(() => closeJefeCashObligation(item.id))
                        }
                  ...
                  {item.calculationStatus === "provisional" && (
                    <p className="mt-2 text-[11px] text-[#E8D5A3]">
                      Provisional: {item.pendingReason}
                    </p>
                  )}
                </div>
              ))}
            ...
            <div className="mt-3 flex gap-2">
              <input
                ...
                onChange={(event) =>
                  setAmounts({ ...amounts, [employeeId]: event.target.value })
                }
                inputMode="decimal"
                placeholder={
                  hasProvisional
                    ? "Confirma los Ubers pendientes"
                    : "Monto recibido"
                }
...
function ServiceList({
  ...
  onDecide: (
    service: Service,
    decision: "aceptar" | "rechazar",
    transport?: "chofer" | "uber",
    bossNotes?: string,
  ) => void;
  onRequestAccept: (service: Service) => void;
  onRequestEdit?: (service: Service) => void;
  onRequestReschedule: (service: Service) => void;
  onRequestLocation: (service: Service) => void;
    ...
    <section className="mt-3 rounded-xl border border-zinc-900 bg-black/50 p-3">
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm font-semibold text-zinc-200">
            ...
            {service.habitacion && (
              <span className="ml-2 font-normal text-zinc-400">{`Hab. ${service.habitacion}`}</span>
            )}
          </p>
          {direccion && (
      ...
      <div className="mt-3 grid grid-cols-3 gap-2">
        <a
          ...
          className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-lg border border-[#C5A55A] px-2 py-2 text-[11px] font-semibold text-[#C5A55A] transition-colors hover:bg-[#C5A55A] hover:text-black"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-lg border border-zinc-800 px-2 py-2 text-[11px] font-semibold text-zinc-300 transition-colors hover:text-white"
        >
          <Navigation size={14} />
          Como llegar
        </a>
...
function ServiceCard({
  ...
  return (
    <article
      className={`rounded-2xl border bg-zinc-950 p-4 sm:p-5 ${pendiente ? "border-[#C5A55A]/50" : "border-zinc-800"}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#8B7635]">
            Servicio {service.id.slice(-6).toUpperCase()}
          </p>
          <h2 className="mt-1 truncate font-heading text-xl font-semibold sm:text-2xl">
            {service.empleada?.nombreArtistico || "Empleada asignada"}
          </h2>
        </div>
      ...
      {programado && service.fechaProgramada && (
        <p className="mt-3 flex items-center gap-2 rounded-xl border border-purple-500/30 bg-purple-500/10 px-3 py-2.5 text-xs font-semibold text-purple-300">
          ...
          Cita pactada:{" "}
          {new Date(service.fechaProgramada).toLocaleString(APP_LOCALE, {
            dateStyle: "short",
            timeStyle: "short",
          })}
        </p>
      ...
      <dl className="mt-3 overflow-hidden rounded-xl border border-zinc-900 bg-black/50">
        {datos.map(([clave, valor]) => (
          <div
            key={clave}
            className="flex items-center justify-between gap-3 border-b border-zinc-900 px-3 py-2.5 text-xs last:border-b-0"
          >
            <dt className="shrink-0 text-zinc-500">{clave}</dt>
            <dd className="truncate text-right font-semibold text-zinc-200">
              {valor}
            </dd>
          </div>
        ))}
      ...
      {service.notasJefe && (
        <div className="mt-3 border-l-2 border-[#C5A55A]/70 bg-black/50 px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#C5A55A]">
            Notas internas
          </p>
      ...
      {pendiente && (
        <button
          type="button"
          disabled={disabled}
          onClick={() => onRequestAccept(service)}
      ...
      */}
      <div className="mt-2.5 grid grid-cols-2 gap-2 sm:grid-cols-3">
        ...
        {sinEmpezar && (
          <SecondaryAction
            onClick={() => onRequestReschedule(service)}
            disabled={disabled}
        ...
        {sinEmpezar && (
          <SecondaryAction
            onClick={() => onRequestLocation(service)}
            disabled={disabled}
...
function etiquetaDia(iso: string) {
  ...
  if (clave === claveDia(new Date(Date.now() - 86_400_000).toISOString()))
    return "Ayer";
    ...
    <div className="flex flex-col gap-5">
      {grupos.map(([dia, delDia]) => (
        <section key={dia} className="flex flex-col gap-2">
          <h3 className="text-[10px] font-bold uppercase tracking-[0.18em] text-zinc-600">
            {dia}
          </h3>
          {delDia.map((service) => (
            <article
            ...
            >
              <button
                type="button"
                onClick={() =>
                  setAbierto(abierto === service.id ? null : service.id)
                }
                aria-expanded={abierto === service.id}
                className="flex w-full items-center gap-3 p-2.5 text-left"
              >
                <span
                  className="block h-10 w-10 shrink-0 rounded-lg border border-zinc-800 bg-cover bg-center"
                  style={
                    service.empleada?.fotoPerfilUrl
                      ? {
                          backgroundImage: `url(${service.empleada.fotoPerfilUrl})`,
                        }
                      : {
                ...
                <span className="min-w-0 flex-1">
                  <span className="block truncate font-heading text-base font-semibold text-white">
                    {service.empleada?.nombreArtistico || "Empleada"}
                  </span>
                  <span className="mt-0.5 block truncate text-[11px] text-zinc-500">
                    ...
                    {service.metodoPago.toUpperCase()}
                  </span>
                </span>
                <span className="flex shrink-0 flex-col items-end gap-1.5">
                  {service.calificacion != null && (
                    <span className="flex items-center gap-1 text-xs font-semibold tabular-nums text-[#E8D5A3]">
                      <Star
                        size={12}
              ...
              {abierto === service.id && (
                <div className="border-t border-zinc-900 p-3.5">
                  {service.empleada && (
                    <EmployeeRatingSummary employee={service.empleada} />
                  )}
...
function EditPendingServiceDialog({
  ...
  const [duracion, setDuracion] = useState<number | string>(
    Number(service.duracionPactadaHoras) || 1,
  );
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-black border border-zinc-800 px-4 py-2.5 rounded-xl text-white outline-none focus:border-[#C5A55A] resize-none"
              placeholder="Instrucciones de llegada o notas..."
            />
          </div>
          <div className="flex gap-2 pt-2">
            <button
              ...
              onClick={onClose}
              disabled={saving}
              className="flex-1 py-2.5 border border-zinc-800 rounded-xl text-xs font-bold uppercase text-zinc-400 hover:text-white"
            >
              Cancelar
            </button>
            <button
              ...
              disabled={saving}
              aria-busy={saving}
              className="flex-1 py-2.5 bg-[#C5A55A] text-black rounded-xl text-xs font-bold uppercase hover:bg-[#D4AF37] disabled:opacity-50"
            >
...
function ReceiptEvidenceList({ service }: { service: Service }) {
  ...
  return (
    <div className="mt-5 rounded-xl border border-zinc-800 bg-black/60 p-4">
      <p className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#C5A55A]">
        <FileCheck2 size={14} />
        Comprobantes
      </p>
      <div className="mt-3 flex flex-wrap gap-2">
        {receipts.map((receipt, index) => (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 rounded-lg border border-zinc-700 px-3 py-2 text-xs text-zinc-300 hover:border-[#C5A55A] hover:text-[#E8D5A3]"
          >
            Comprobante {index + 1} ·{" "}
            {(receipt.estado ?? "sin estado").replaceAll("_", " ")}
...
function AcceptServiceDialog({
  ...
  disabled: boolean;
  onClose: () => void;
  onAccept: (transport: "chofer" | "uber", notes?: string) => void;
}) {
  const [notes, setNotes] = useState("");
  const sameLocation = Boolean(
    previousService?.presetLocationId &&
    previousService.presetLocationId === service.presetLocationId,
  );
  return (
    <div
      className="fixed inset-0 z-[60] flex items-end justify-center bg-black/80 p-3 backdrop-blur-sm sm:items-center"
    ...
    >
      <section
        ...
        className="w-full max-w-lg rounded-2xl border border-[#C5A55A]/50 bg-[#050505] p-5 shadow-2xl sm:p-7"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2
              ...
              className="mt-1 font-heading text-3xl"
            >
              {service.empleada?.nombreArtistico || "Empleada"}
            </h2>
          </div>
        ...
        <div className="mt-5 rounded-xl border border-zinc-800 bg-black p-4 text-sm text-zinc-400">
          {sameLocation ? (
            <>
        ...
        <label className="mt-5 block">
          ...
          <textarea
            ...
            maxLength={2000}
            rows={4}
            placeholder="Escribe instrucciones internas o continúa sin notas"
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-2">
          {sameLocation ? (
            <button
              ...
              className="sm:col-span-2 rounded-xl bg-[#C5A55A] px-4 py-3 text-xs font-bold uppercase tracking-wider text-black disabled:opacity-50"
            >
              Aceptar en la misma ubicación
            </button>
          ) : (
            <>
              <button
...
function EmployeeRatingSummary({ employee }: { employee: Employee }) {
  if (employee.promedioCalificacion == null)
    return <p className="mt-2 text-xs text-zinc-500">Sin valoraciones</p>;
  return (
    <p className="mt-2 flex items-center gap-1.5 text-xs text-zinc-400">
      <Star size={13} className="fill-[#C5A55A] text-[#C5A55A]" />
      <span className="font-semibold text-[#E8D5A3]">
        {Number(employee.promedioCalificacion).toFixed(2)}
      </span>
      <span>
        {employee.totalServiciosValorados}{" "}
        {employee.totalServiciosValorados === 1
          ? "servicio valorado"
          : "servicios valorados"}
      </span>
    </p>
...
function resumenEmpleada(employee: Employee) {
   ...
   */
  const sinTelegram =
    employee.usuario && !employee.usuario.telegramChatId
      ? " · Sin Telegram"
      : "";
  ...
  if (!employees.length)
    return (
      <div className="rounded-2xl border border-dashed border-zinc-800 py-16 text-center text-sm text-zinc-500">
        No hay empleadas que coincidan con la búsqueda.
      </div>
    );
  return (
    <ul className="flex flex-col gap-2">
      {employees.map((employee) => (
        <li
          key={employee.id}
          className={`flex items-center gap-2 rounded-2xl border bg-zinc-950 pr-2 transition-colors ${employee.disponible ? "border-[#C5A55A]/45" : "border-zinc-800"}`}
        >
          ...
          <button
            ...
            className="flex min-w-0 flex-1 items-center gap-3 rounded-l-2xl py-2.5 pl-2.5 text-left"
          >
            <span className="relative shrink-0">
              <span
                className="block h-12 w-12 rounded-xl border border-zinc-800 bg-cover bg-center"
                style={fondoEmpleada(employee)}
              />
              <span
                className={`absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-zinc-950 ${employee.availabilityStatus === "ocupada" ? "bg-[#8B7635]" : employee.disponible ? "bg-emerald-400" : "bg-zinc-700"}`}
              />
            </span>
            <span className="min-w-0 flex-1">
              <span className="flex items-center gap-2">
                <span className="min-w-0 flex-1 truncate font-heading text-[17px] font-semibold leading-tight text-white">
                  {employee.nombreArtistico}
                </span>
              ...
              <span
                className={`mt-1 block truncate text-[11px] ${employee.availabilityStatus === "ocupada" ? "text-[#E8D5A3]" : "text-zinc-500"}`}
              >
                {resumenEmpleada(employee)}
              </span>
            </span>
          </button>
          <button
            ...
            aria-label={
              employee.disponible
                ? `Marcar a ${employee.nombreArtistico} como no disponible`
                : `Marcar a ${employee.nombreArtistico} como disponible`
            }
            title={
      ...
      <div className="w-full rounded-t-3xl border border-b-0 border-zinc-800 bg-[#090909] p-5 pb-8 shadow-2xl sm:max-w-md sm:rounded-2xl sm:border-b sm:pb-5">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3.5">
            <span
              className="block h-16 w-16 shrink-0 rounded-2xl border border-[#C5A55A]/45 bg-cover bg-center"
              style={fondoEmpleada(employee)}
            />
            <div className="min-w-0">
              <h3 className="truncate font-heading text-2xl font-semibold text-white">
                {employee.nombreArtistico}
              </h3>
              <p className="mt-1 flex items-center gap-1 text-xs text-zinc-500">
                <MapPin size={12} />
                {employee.ubicacionLat ? "Ubicación recibida" : "Sin ubicación"}
              </p>
        ...
        >
          ...
          {sinLeer > 0
            ? `Leer sus ${sinLeer === 1 ? "mensaje" : "mensajes"}`
            : "Escribirle"}
        ...
        <div className="mt-2.5 grid grid-cols-2 gap-2.5">
          <button
            type="button"
            onClick={onPhotos}
            className="flex h-24 flex-col items-start justify-between rounded-xl border border-zinc-800 bg-zinc-900/60 p-3.5 text-left transition-colors hover:border-[#C5A55A]"
          >
...
function EstadoDeJornada({ employee }: { employee: Employee }) {
  ...
  async function preguntar() {
    ...
    setPreguntado(true);
    toast.success(
      "Se lo preguntamos. Te llega la respuesta en cuanto conteste",
    );
  }
...
function ServiceRating({ service }: { service: Service }) {
  return (
    <div className="mt-5 rounded-xl border border-zinc-800 bg-black/60 p-4">
      ...
      {service.calificacion == null ? (
        <p className="mt-2 text-sm text-zinc-500">Sin valoración</p>
      ) : (
        <div
          className="mt-2 flex items-center gap-1"
          aria-label={`${service.calificacion} de 5 estrellas`}
        >
          {[1, 2, 3, 4, 5].map((value) => (
            <Star
              key={value}
              size={18}
              className={
                value <= service.calificacion!
                  ? "fill-[#C5A55A] text-[#C5A55A]"
                  : "text-zinc-700"
              }
            />
          ))}
          <span className="ml-2 text-sm font-semibold text-[#E8D5A3]">
            {service.calificacion}/5
          </span>
        </div>
      )}
      {service.comentariosCalificacion && (
        <blockquote className="mt-3 border-l-2 border-[#C5A55A]/60 pl-3 text-sm leading-relaxed text-zinc-300">
          {service.comentariosCalificacion}
        </blockquote>
      )}
    </div>
...
function TransportPanel({
  ...
  return (
    <section className="mt-6 space-y-4 border-t border-zinc-800 pt-6">
      <header>
        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#C5A55A]">
          Transporte del servicio
        </p>
        <p className="mt-1 text-sm text-zinc-500">
          Gestiona por separado los viajes de ida y regreso.
        </p>
      </header>
...
function TripCard({
  ...
  const canChangeTransport =
    (!trip.choferId || trip.estado === "notificado") &&
    ["notificado", "aceptado", "llegado"].includes(trip.estado);
   ...
   */
  const retenido =
    trip.tipo === "ida" && Boolean(service && esperandoAlistado(service));
  const changeButton = canChangeTransport && (
    <button
      type="button"
      onClick={() =>
        onRun(
          () =>
            changeTripTransport(
              trip.id,
              trip.proveedorTransporte === "uber" ? "chofer" : "uber",
            ),
          "Método de transporte actualizado",
        )
      }
      className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-[#C5A55A] px-3 py-3 text-xs font-semibold text-[#C5A55A]"
    >
      <Repeat2 size={15} />
      Cambiar a {trip.proveedorTransporte === "uber" ? "chofer" : "Uber"}
    </button>
  );
  return (
    <article className="overflow-hidden rounded-xl border border-zinc-800 bg-black">
      ...
      {trip.proveedorTransporte === "uber" ? (
        <div className="space-y-5 p-4">
          {retenido ? (
            <div className="rounded-xl border border-[#C5A55A]/40 bg-[#C5A55A]/5 p-4 text-xs text-[#E8D5A3]">
              <p className="flex items-center gap-2 font-semibold">
                <Hourglass size={15} className="shrink-0" />
                Esperando a que se aliste
              </p>
              <p className="mt-1.5 leading-relaxed text-zinc-400">{`En cuanto ${service?.empleada?.nombreArtistico || "ella"} avise que está lista aparece aquí el botón de pedir el Uber. Si no puedes esperar, cambia el viaje a chofer.`}</p>
            </div>
          ) : (
            <a
              href={uberDeeplink}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#C5A55A] px-4 py-3 text-xs font-bold uppercase tracking-wider text-black"
            >
              <Smartphone size={16} />
              Pedir Uber
            </a>
          ...
          {hasScreenshot ? (
            <div className="rounded-lg border border-[#C5A55A]/40 bg-[#C5A55A]/5 p-3 text-xs text-[#E8D5A3]">
              Captura recibida
              {trip.uberScreenshotUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-3 flex items-center gap-2 font-semibold text-[#C5A55A]"
                >
                  Ver captura <ExternalLink size={12} />
                </a>
              )}
            </div>
          ) : (
            <UberScreenshotUploader tripId={trip.id} onRefresh={onRefresh} />
          )}
          <UberFareEditor trip={trip} onRefresh={onRefresh} />
          {changeButton}
        </div>
      ) : (
...
function UberFareEditor({
  ...
  const canEditFare =
    ...
    Boolean(trip.uberScreenshotUrl || trip.telegramUberFileId);
  ...
  async function saveFare() {
    ...
    if (!Number.isFinite(amount) || amount <= 0)
      return toast.error("Ingresa una tarifa válida");
    setSaving(true);
  ...
  if (!editing && hasFare)
    return (
      <div className="flex items-center justify-between gap-3 rounded-lg border border-zinc-800 bg-zinc-950 p-3">
        <div className="flex items-center gap-2">
          <CircleDollarSign size={18} className="text-[#C5A55A]" />
          <div>
            <p className="text-[10px] uppercase tracking-wider text-zinc-500">
              Tarifa final
            </p>
            <p className="font-heading text-xl text-[#E8D5A3]">
              ${storedFare.toFixed(2)}
            </p>
          </div>
        </div>
        {canEditFare && (
          <button
            type="button"
            onClick={() => {
              setFare(String(storedFare));
              setEditing(true);
            }}
            className="inline-flex items-center gap-1 rounded-lg border border-[#C5A55A] px-3 py-2 text-xs font-semibold text-[#C5A55A]"
          >
            <Pencil size={13} />
            Cambiar tarifa
          </button>
  ...
  return (
    <div className="space-y-2 border-t border-zinc-900 pt-4">
      <label
        ...
        htmlFor={`fare-${trip.id}`}
      >
        Tarifa final del viaje de {trip.tipo}
      </label>
      <div className="flex gap-2">
        <input
          ...
          inputMode="decimal"
          placeholder="0.00"
          className="min-w-0 flex-1 rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-3 text-sm outline-none focus:border-[#C5A55A]"
        />
        <button
          type="button"
          disabled={saving}
          aria-busy={saving}
          onClick={saveFare}
          className="rounded-lg bg-[#C5A55A] px-4 py-3 text-xs font-semibold text-black disabled:opacity-50"
        >
        ...
        {hasFare && (
          <button
            ...
            className="rounded-lg border border-zinc-800 px-3 py-3 text-xs text-zinc-400 disabled:opacity-50"
          >
            Cancelar
          </button>
        )}
...
function ChatPanel({
  ...
  async function send() {
    ...
    if (!value) return;
    const result = await sendServiceMessage(service.id, value);
  ...
  /*
   * La caja se levanta por encima de la barra de abajo.
   *
   * Con `bottom-3` quedaba a doce pixeles del borde, y ahi vive la barra de
   * navegacion del panel --fija, del mismo z-index-- mas el indicador de inicio
   * del iPhone. La fila de escribir caia justo detras: el chat parecia no tener
   * campo de texto, cuando lo que pasaba es que no se veia. La barra mide el
   * alto de sus botones mas su propio hueco de area segura, y a partir de `md`
   * no existe, asi que ahi se vuelve al margen de siempre.
   *
   * `dvh` y no `vh`: en el movil `vh` mide la ventana sin la barra del
   * navegador, asi que el alto maximo se pasaba de lo que de verdad se ve.
   */
  return (
    <aside className="fixed inset-x-3 bottom-[calc(4.5rem+env(safe-area-inset-bottom))] z-50 flex max-h-[75dvh] flex-col rounded-2xl border border-[#C5A55A]/60 bg-[#050505] shadow-2xl md:bottom-3 md:left-auto md:right-6 md:w-[420px]">
      <header className="flex items-center justify-between border-b border-zinc-800 p-4">
        <div>
          <p className="font-heading text-xl">Conversación</p>
      ...
      {hasPriorContext && (
        <div className="border-b border-zinc-800 bg-[#C5A55A]/5 px-4 py-3 text-xs leading-relaxed text-[#E8D5A3]">
          Esta conversación comenzó antes de que el servicio fuera creado. El
          historial previo se muestra para conservar el contexto.
        </div>
      )}
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        ...
        {messages.map((message) => {
          const presentation = senderPresentation[message.emisor];
          return (
            <div
              key={message.id}
              className={`max-w-[85%] rounded-xl px-3 py-2 text-sm ${presentation.className}`}
            >
      ...
      <div className="flex gap-2 border-t border-zinc-800 p-3">
        <textarea
          ...
          className="min-h-11 flex-1 resize-none rounded-xl border border-zinc-800 bg-black px-3 py-2 text-sm outline-none focus:border-[#C5A55A]"
        />
        <button
          onClick={send}
          className="rounded-xl bg-[#C5A55A] px-4 text-black"
          aria-label="Enviar"
        >
          <Send size={18} />
        </button>
...
function ActionButton({
  ...
  return (
    <button
      {...props}
      className={`inline-flex items-center gap-1 rounded-lg px-4 py-3 text-xs font-bold uppercase tracking-wider disabled:opacity-50 ${outline ? "border border-[#C5A55A] text-[#C5A55A]" : "bg-[#C5A55A] text-black"}`}
    >
      {children}
    </button>
  );
