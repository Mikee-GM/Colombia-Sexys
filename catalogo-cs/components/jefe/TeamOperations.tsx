...
import { Award, Banknote, Ban, Car, Check, CircleDollarSign, Clock3, ExternalLink, FileCheck2, MapPin, MessageCircle, Pencil, Repeat2, Search, Send, Smartphone, Star, UserRoundCheck, UserRoundX, X } from "lucide-react";
...
export default function TeamOperations({ initialEmployees, initialServices, initialCashSummary, initialGroupRequests }: { initialEmployees: Employee[]; initialServices: Service[]; initialCashSummary: CashObligationSummary; initialGroupRequests: GroupServiceRequest[] }) {
  ...
  function cancelService(service: Service) {
    if (!window.confirm("¿Confirmas que deseas cancelar este servicio? Se quitará del panel operativo.")) return;
    startTransition(async () => {
      const result = await cancelJefeService(service.id);
      if (!result.success) {
        toast.error(result.error);
        return;
      }
...
function TripCard({ trip, onRefresh, onRun }: { trip: Trip; onRefresh: () => Promise<void>; onRun: (action: () => Promise<{ success: boolean; error?: string }>, success: string) => Promise<void> }) {
  ...
  return <article className="overflow-hidden rounded-xl border border-zinc-800 bg-black"><header className="flex items-center justify-between gap-3 border-b border-zinc-800 px-4 py-3"><div className="flex items-center gap-2"><span className="rounded-lg bg-[#C5A55A]/10 p-2 text-[#C5A55A]"><Car size={17} /></span><div><p className="text-sm font-semibold capitalize">Viaje de {trip.tipo}</p><p className="text-[10px] uppercase tracking-wider text-zinc-600">{trip.proveedorTransporte}</p></div></div><span className="rounded-full border border-zinc-800 px-2.5 py-1 text-[10px] uppercase tracking-wider text-zinc-400">{trip.estado}</span></header>{trip.proveedorTransporte === "uber" ? <div className="space-y-5 p-4"><div className={`rounded-lg border p-3 text-xs ${hasScreenshot ? "border-[#C5A55A]/40 bg-[#C5A55A]/5 text-[#E8D5A3]" : "border-zinc-800 text-zinc-500"}`}>{hasScreenshot ? "Captura de la empleada recibida" : "Esperando la captura del resumen del viaje"}{trip.uberScreenshotUrl && <a href={trip.uberScreenshotUrl} target="_blank" rel="noopener noreferrer" className="mt-3 flex items-center gap-2 font-semibold text-[#C5A55A]">Ver captura <ExternalLink size={12} /></a>}</div><UberFareEditor trip={trip} onRefresh={onRefresh} />{changeButton}</div> : <div className="space-y-4 p-4"><p className="text-sm text-zinc-500">El viaje será gestionado por un chofer interno.</p>{changeButton}</div>}</article>;
}
