...
export default async function BitacoraChatPage() {
  ...
  const [services, unlinkedSessions] = await Promise.all([
    optionalSource(getServicesForChatReviewAction(), [], contexto),
    optionalSource(getUnlinkedSessionsAction(), [], contexto),
  ]);
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm text-zinc-500">
          Herramienta interna para revisar el flujo completo de una
          conversación, desde el primer mensaje hasta el último, se haya
          concretado en un servicio o no. No compartir el enlace fuera del
          equipo de desarrollo.
        </p>
      </div>
      <ConversationLogViewer
        initialServices={services}
        initialUnlinkedSessions={unlinkedSessions}
      />
    </div>
  );
}
