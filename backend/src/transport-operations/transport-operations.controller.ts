...
export class TransportOperationsController {
  constructor(
    private readonly service: TransportOperationsService,
  ...
  @Patch('configuration') update(
    @Body() dto: UpdateTransportSettingDto,
    @Req() req: any,
  ) {
  ...
  @Patch('locations/:id') edit(
    @Param('id') id: string,
    @Body() dto: SavePresetLocationDto,
  ) {
    return this.service.updateLocation(id, dto);
  }
  @Delete('locations/:id') remove(@Param('id') id: string) {
    return this.service.removeLocation(id);
  }
  @Get('cash-obligations') @Roles('admin', 'jefe') cash(
    @Req() req: any,
    @Query('employeeId') employeeId?: string,
  ) {
    return this.settlements.cashSummary(req.user, employeeId);
  }
  @Post('cash-payments') @Roles('admin', 'jefe') cashPayment(
    @Body() dto: CashPaymentDto,
    @Req() req: any,
  ) {
    return this.settlements.registerCashPayment(
      dto.employeeId,
      dto.amount,
      dto.note,
      req.user,
  ...
  @Post('cash-obligations/:id/close') @Roles('admin', 'jefe') closeCash(
    ...
    return this.settlements.closeCashObligation(id, req.user);
  }
  @Get('driver-settlements') driverReport(
    @Query() period: SettlementPeriodDto,
  ) {
    return this.settlements.driverReport(period.startDate, period.endDate);
  }
  @Post('driver-settlements/:driverId/pay') settleDriver(
    @Param('driverId') driverId: string,
    @Body() period: SettlementPeriodDto,
    @Req() req: any,
  ) {
    return this.settlements.settleDriver(
      driverId,
      period.startDate,
      period.endDate,
      req.user.id,
